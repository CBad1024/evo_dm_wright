...
class DrugSelector:
    ...
    def get_qs(self):
        # print(f"TRAIN_INPUT: {self.hp.TRAIN_INPUT}")
        # print(f"Environment shape: {self.env.ENVIRONMENT_SHAPE}")
        ...
        #     print(f"Tensor shape: {tens.shape}")
...
def compute_optimal_action(agent, policy, step, prev_action=False):
    ...
    if prev_action:
        if agent.env.TRAIN_INPUT == "state_vector":
            ...
            action = policy[index][
                         int(agent.env.prev_action) - 1] + 1  # plus one because I made the bad decision to force the actions to be 1-indexed once upons a time
    else:
        action = policy[index][
                     step] + 1  # plus one because I made the bad decision to force the actions to be 1,2,3,4 once upon a time
            ...
            if np.random.random() > agent.hp.epsilon:
                ...
                if naive:
                    ...
                    if wf:
                        ...
                        agent.env.action = np.argmax(
                            agent.get_qs())  # plus one because of the stupid fucking indexing system
            else:
                # Get random action
                if standard_practice and not wf:
                    # Only change the action if fitness is above 0.9
